...
bool LoadAverage::GetLoadAverage() {
    auto load_average_wrapper = detail::GetSystemLoadAverage();
    if (!load_average_wrapper) {
        return false;
    }
    auto load_average = load_average_wrapper.value();
    auto cpu_count_wrapper = detail::GetCpuCount();
    if (!cpu_count_wrapper) {
        return false;
    }
    auto cpu_count = cpu_count_wrapper.value();
    auto load_percent = (load_average * 100.0) / static_cast<double>(cpu_count);
    details_.push_back(make_tuple("load average", format("{:.2f}% ({:.2f})", load_percent, load_average)));
