...
#include "common/include/portability.h"
...
private:
    static ScopeGuardImplBase makeFailsafe(std::true_type, const void *) noexcept { return makeEmptyScopeGuard(); }
    ...
    void execute() noexcept(InvokeNoexcept) {
        if (InvokeNoexcept) {
            ...
            auto catcher = []() -> R {
                warnAboutToCrash(), std::terminate();
            };
...
makeGuard(F &&f) noexcept(noexcept(detail::ScopeGuardImplDecay<F, true>(static_cast<F &&>(f)))) {
    return detail::ScopeGuardImplDecay<F, true>(static_cast<F &&>(f));
}

namespace detail {

#if defined(FOLLY_EXCEPTION_COUNT_USE_CXA_GET_GLOBALS) || defined(FOLLY_EXCEPTION_COUNT_USE_GETPTD) || \
...
ScopeGuardForNewException<typename std::decay<FunctionType>::type, true> operator+(detail::ScopeGuardOnFail,
                                                                                   FunctionType &&fn) {
...
template<typename FunctionType>
ScopeGuardForNewException<typename std::decay<FunctionType>::type, false> operator+(ScopeGuardOnSuccess,
                                                                                    FunctionType &&fn) {
...
#define SCOPE_EXIT auto FB_ANONYMOUS_VARIABLE(SCOPE_EXIT_STATE) = ::folly::detail::ScopeGuardOnExit() + [&]() noexcept
    ...
    defined(FOLLY_EXCEPTION_COUNT_USE_STD)
#    define SCOPE_FAIL \
        auto FB_ANONYMOUS_VARIABLE(SCOPE_FAIL_STATE) = ::folly::detail::ScopeGuardOnFail() + [&]() noexcept

#    define SCOPE_SUCCESS \
        auto FB_ANONYMOUS_VARIABLE(SCOPE_SUCCESS_STATE) = ::folly::detail::ScopeGuardOnSuccess() + [&]()
#endif // native uncaught_exception() supported
